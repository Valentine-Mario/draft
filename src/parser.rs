...
//Program is a sequence of statements and expressions. To make life easier in the future we will close every expression 
//in an anonymous function (we'll use this during JIT compilation). So, there are two types of items in the program after such a closure: declarations and definitions. 
//Declarations are just function prototypes, when definitions are function prototypes combined with a function body.
...
//Helper parsing functions will accept unparsed tokens as their input. They will have three possible results:
...
//create a function that fills this map with some operators
...
fn parse_prototype(tokens : &mut Vec<Token>, _settings : &mut ParserSettings) -> PartParsingResult<Prototype> {
    ...
    PartParsingResult::Good(Prototype{name: name, args: args}, parsed_tokens)
}
...
//First, we parse the name (it will be the name of a variable or function to call). Then we look at the next token. 
//If it is an opening parenthesis, then we have a function call.
// If it is any other token, then we have a variable referenced and we can return a VariableExpr already.
